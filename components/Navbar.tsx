...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-[var(--line-subtle)] bg-[var(--header-bg)] backdrop-blur-md">
          ...
          <nav className="hidden lg:flex items-center gap-2">
            {[
              ...
              <Link
                key={link.name}
                href={link.href}
                className="inline-flex h-9 items-center justify-center rounded-md px-4 text-sm font-medium text-[var(--text-soft)] transition hover:bg-[var(--surface-2)] hover:text-[var(--text-strong)]"
              >
                {link.name}
              </Link>
            ))}
        ...
        <div className="flex items-center gap-3">
          ...
          {user ? (
            <div className="flex items-center gap-3">
              ...
              {/* Notifications */}
              <button className="relative h-9 w-9 flex items-center justify-center rounded-md text-[var(--text-soft)] hover:bg-[var(--surface-2)] transition-colors">
              ...
              {/* Settings */}
              <button className="h-9 w-9 flex items-center justify-center rounded-md text-[var(--text-soft)] hover:bg-[var(--surface-2)] transition-colors">
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            <div className="flex items-center gap-2">
              <Link
                href="/login"
                className="h-9 px-4 flex items-center justify-center text-sm font-semibold text-[var(--text-soft)] hover:text-[var(--text-strong)] transition-all"
              >
                Log In
              </Link>
              <Link
